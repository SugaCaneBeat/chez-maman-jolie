...
export default function OrderStatusView({
  ...
  /* État de la notification automatique WhatsApp */
  const [autoNotifyTriggered, setAutoNotifyTriggered] = useState(false);
  ...
  /* ── Auto-notification WhatsApp après paiement confirmé ──
     Quand on revient de SumUp et que le statut passe à "paid",
     on ouvre automatiquement WhatsApp (une seule fois par commande,
     suivi via localStorage pour éviter les rejeux sur refresh). */
  useEffect(() => {
    if (!justPaid) return;
    if (order.status !== "paid" && order.status !== "confirmed" && order.status !== "preparing") return;
    if (autoNotifyTriggered) return;

    const storageKey = `notify-sent-${order.id}`;
    if (typeof window !== "undefined" && window.localStorage.getItem(storageKey) === "1") {
      setAutoNotifyTriggered(true);
      return;
    }

    /* Petit délai pour laisser le bandeau "Paiement confirmé" être visible */
    const t = setTimeout(() => {
  ...
  }, [justPaid, order.status, order.id, whatsappNotifyUrl, autoNotifyTriggered]);
      ...
      <main className="max-w-xl mx-auto px-5 py-8 space-y-6">
        ...
        {justPaid && (
          <div className={`rounded-[5px] p-4 border ${
            ...
            <div className="flex items-start gap-3">
              <svg aria-hidden="true" className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
                order.status === "paid" || order.status === "confirmed" || order.status === "preparing"
                  ? "text-emerald-400"
                  : "text-amber-400"
              ...
              <div className="flex-1">
                ...
                <p className="text-white/60 text-xs mt-0.5">
                  {order.status === "pending"
                    ...
                    : autoNotifyTriggered
                      ? "Le restaurant a été notifié sur WhatsApp."
                      : "Notification du restaurant sur WhatsApp dans un instant…"}
                </p>
              </div>
            </div>

            {/* Bouton WhatsApp visible si paiement confirmé (relance si bloqueur popup) */}
            {(order.status === "paid" || order.status === "confirmed" || order.status === "preparing") && (
              <a
                href={whatsappNotifyUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-3 flex items-center justify-center gap-2 w-full bg-[#25D366] hover:bg-[#20BD5A] text-white font-bold py-3 rounded-[5px] text-sm transition-all hover:scale-[1.02]"
              >
                ...
                {autoNotifyTriggered ? "Renvoyer la notification au restaurant" : "Notifier le restaurant maintenant"}
              </a>
            )}
          </div>
        )}
        ...
        >
          <div className="flex items-center justify-center gap-2 mb-2">
            {cancelled ? (
              <svg aria-hidden="true" className="w-5 h-5 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
