...
export default function AddToCartButton({ item, className = "" }: AddToCartButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      className={`w-11 h-11 rounded-[5px] flex items-center justify-center transition-all duration-300 ${
        clicked
    ...
    >
      {clicked ? (
        <svg aria-hidden="true" className="w-5 h-5 text-dark" fill="none" stroke="currentColor" viewBox="0 0 24 24">
