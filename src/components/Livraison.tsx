    ...
    <section id="livraison" className="py-20 sm:py-32 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8 relative z-10">
        ...
        <ScrollAnimation delay={100}>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 sm:gap-6 mb-20">
            {data.tunnel.map((s, i) => (
        ...
        <ScrollAnimation delay={200}>
          <div className="glass rounded-[5px] overflow-hidden mb-8">
              ...
              <div>
                ...
                {data.depart && (
                  <p className="text-xs text-white/40 mt-1 flex items-center gap-1.5">
                    <svg aria-hidden="true" className="w-3.5 h-3.5 text-primary/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="divide-y divide-white/5">
              {data.zones.map((zone) => (
                <div key={zone.name} className="flex flex-col sm:flex-row sm:items-start justify-between px-6 sm:px-8 py-5 hover:bg-white/3 transition-colors gap-2">
                  <div>
                    <h4 className="font-semibold text-white text-sm sm:text-base">{zone.name}</h4>
                    <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs sm:text-sm">
                      <span className="text-primary/80">{zone.frais}</span>
                      {zone.fraisBas && (
                        <span className="text-white/30">&middot; {zone.fraisBas}</span>
                      )}
                    </div>
                  </div>
                  <span className="text-primary font-bold mt-1 sm:mt-0 text-sm flex-shrink-0">{zone.delai}</span>
                </div>
              ))}
            </div>
            {data.notePickup && (
        ...
        <ScrollAnimation delay={400}>
          <div className="text-center mt-12 flex flex-col sm:flex-row items-center justify-center gap-3">
            <CommanderButton className="group inline-flex items-center gap-3 bg-gradient-to-r from-primary to-primary-light text-dark font-bold px-8 py-4 rounded-[5px] text-base transition-all hover:scale-105 shadow-lg shadow-primary/20">
            ...
            >
              <svg aria-hidden="true" className="w-4 h-4 text-[#25D366]" fill="currentColor" viewBox="0 0 24 24">
